<!--Page ou on aura le visuelle des encheres ajouter apres utilisation du formumaire d'ajout-->

<div class="container_fluid pb-5 mb-5">
  <!--AJOUT DU TIRE-->
  <div class="container d-flex justify-content-center">
    <h2>Enchère en cours</h2>
  </div>

  <!--AJOUT VISUEL DES ENCHERES (BRUT)-->

  <div class="container ">

    <!--Première ranger de 3 carte-->
    <div class="card-deck mt-5">
  <div class="card ">
    <img src="ressources/img/no_image.png" class="card-img-top" alt="...">
    <div class="card-body">
      <h4 class="card-title">NOM PRODUIT</h4>

      <h5 class = "card-title">Prix :</h5>

      <p class="card-text">This is a longer card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
      <!--Ajout du bouton enchérire-->
      <form>
      <button type="button" class="btn btn-success">Enchérir</button>
</form>
    </div>
  </div>
  <div class="card ">
    <img src="ressources/img/no_image.png" class="card-img-top" alt="...">
    <div class="card-body">
      <h4 class="card-title">NOM PRODUIT</h4>

      <h5 class = "card-title">Prix :</h5>

      <p class="card-text">This is a longer card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
      <!--Ajout du bouton enchérire-->
      <form>
      <button type="button" class="btn btn-success">Enchérir</button>
</form>
    </div>
  </div>
  <div class="card ">
    <img src="ressources/img/no_image.png" class="card-img-top" alt="...">
    <div class="card-body">
      <h4 class="card-title">NOM PRODUIT</h4>

      <h5 class = "card-title">Prix :</h5>

      <p class="card-text">This is a longer card with czsxcszdvcscscsdcsdcfzdcvzdvedvzdvedvcdjzhqbfjzheibfjhgebjhvfbcjzhedgfbjszdvdvzevzefzevzfesupporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
      <!--Ajout du bouton enchérire-->
      <form>
      <button type="button" class="btn btn-success">Enchérir</button>
</form>
    </div>
  </div>

    <!--Seconde rangé de trois enchères-->
    
    <div class="card-deck mt-5">
  <div class="card ">
    <img src="..." class="card-img-top" alt="...">
    <div class="card-body">
    <h5 class="card-title">NOM PRODUIT</h5>
      <p class="card-text">This is a longer card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p>
       <!--Ajout du bouton enchérire-->
       <form>
      <button type="button" class="btn btn-success">Enchérir</button>
</form>
    </div>
  </div>
  <div class="card">
    <img src="..." class="card-img-top" alt="...">
    <div class="card-body"><h5 class="card-title">NOM PRODUIT</h5>
      <p class="card-text">This card has supporting text below as a natural lead-in to additional content.</p>
       <!--Ajout du bouton enchérire-->
       <form>
      <button type="button" class="btn btn-success">Enchérir</button>
</form>
    </div>
  </div>
  <div class="card">
    <img src="..." class="card-img-top" alt="...">
    <div class="card-body">
    <h5 class="card-title">NOM PRODUIT</h5>
      <p class="card-text">This is a wider card with supporting text below as a natural lead-in to additional content. This card has even longer content than the first to show that equal height action.</p>
      <!--Ajout du bouton enchérire-->
      <form>
      <button type="button" class="btn btn-success">Enchérir</button>
</form>
      
    </div>
  </div>
</div>







  </div>





</div>